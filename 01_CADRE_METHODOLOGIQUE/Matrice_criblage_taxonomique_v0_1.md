...
```text
ne pas demander d'abord a quelle famille appartient une grandeur ;
...
La methode anterieure risquait parfois de transformer des categories commodes en familles trop vite.

Trois risques sont particulierement vises :

```text
Relation comme famille fourre-tout,
fonction contextuelle prise pour une famille,
architecture prise pour une famille superieure.
```
...
| Forme logique | Quelle forme est stabilisee ? | Valeur, rapport, ecart, matrice, phase, relation, borne, observable |
| Famille candidate | Quelle fonction principale semble dominante ? | Couplage, Echelle, Raccordement, Orientation, Convention, Relation sous audit |
| Test de famille | La fonction est-elle irreductible au rang 0 ou au role ? | Confirmee, qualifiee, deplacee, refusee |
...
### 5. Test de resistance d'une famille

Une famille candidate ne doit etre conservee que si elle resiste aux questions suivantes.

| Question | Si oui | Si non |
|---|---|---|
| La grandeur a-t-elle une fonction principale claire ? | Continuer le test | Ne pas classer en famille |
| Cette fonction est-elle plus qu'une forme logique ? | Famille candidate possible | Revenir au rang 0 |
| Cette fonction est-elle plus qu'un role contextuel ? | Famille candidate possible | Deplacer en role |
| Cette fonction reste-t-elle lisible hors d'une architecture particuliere ? | Famille robuste possible | Role architectural ou architecture |
...
```text
famille robuste
famille conservee mais qualifiee
...
### 6. Test special de Relation

Relation est une famille faible sous audit.
...
| Question | Effet |
|---|---|
| La liaison est-elle la fonction principale, ou seulement la forme logique ? | Si forme logique seulement, sortir de Relation |
...
| Le cas exige-t-il une architecture pour etre compris ? | Si oui, traiter l'architecture avant la famille |
| Que perd-on si l'etiquette Relation disparait ? | Si presque rien, Relation est refusee |

Decisions possibles :

| Ancien usage de Relation | Decision v1.3 |
|---|---|
| Rapport pur | Rang 0 : rapport, puis fonction a tester |
| Relation d'etat | Role contextuel d'etat |
| Composition metrologique | Convention + role compositionnel |
| Relation constitutive Saveur-Higgs | Architecture constitutive a expliciter |
| Normalisation cosmologique | Role contextuel de densite normalisee |

Formule :

```text
Relation ne devient acceptable que si elle explique la fonction,
pas seulement la forme de l'ecriture.
```

### 7. Test de role contextuel

Un role contextuel n'est pas une famille.
...
```text
le role peut etre indispensable sans devenir une famille.
```
...
| Question | Critere |
|---|---|
| Plusieurs familles ou roles sont-ils solidaires ? | Il faut plus qu'une simple liste |
...
| La structure resiste-t-elle a un contre-classement par famille simple ? | Sinon, revenir au rang 1 ou 2 |
...
Rang 1 - famille candidate :
Test de resistance de famille :
...
|---|---|
| `m_f = y_f v / sqrt(2)` | Forme relationnelle, mais architecture Saveur-Higgs a documenter avant de conclure Relation |
...
| `Delta m^2` | Ecart spectral et dephasage oscillatoire ; tester Orientation, Echelle ou role de phase avant Relation |
