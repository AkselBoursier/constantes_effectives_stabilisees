...
```text
statut : cycle physique ouvert ;
code : CP-1 ;
objet : seuil electrofaible rendu lisible par le mecanisme BEH ;
noyau : `v` comme echelle de regime ;
controle : masses faibles et acces par `G_F` ;
registre : froid, positif, preparatoire a une synthese ulterieure ;
statut probatoire : P2/P3 interne, avec formules de rang P5 pour
la qualification propre du corpus.
```

Ce cycle ouvre le point aveugle identifie dans la mise a jour
intellectuelle :
...
02_CYCLES_PHYSIQUES/05_Cycle_saveur_Higgs/00_Sources_docx/Source_docx_Fiche_v_v0_1.md
02_CYCLES_PHYSIQUES/05_Cycle_saveur_Higgs/00_Sources_docx/Source_docx_Fiche_Yukawa_v0_1.md
...
et comment `v`, les masses faibles, les Yukawa et les regimes
d'acces y prennent chacun leur rang ?
```
...
```text
1. un etat electroweak a basse energie ;
2. une echelle `v` ;
3. des masses faibles ;
4. une route d'acces par `G_F` et les mesures electroweak ;
5. une transition vers l'architecture Saveur-Higgs.
```

### 2. Formule positive d'ouverture

Formule directrice :

```text
Le seuil BEH installe le regime dans lequel `v` fonctionne comme
noyau d'echelle, les masses faibles comme temoins phenomenologiques
et les Yukawa comme relais vers la differenciation fermionique.
```

Cette formule tient trois niveaux ensemble :
...
```text
potentiel scalaire
-> etat fondamental structure
-> disponibilite de `v`
-> recombinaison electroweak
-> masses `W` et `Z`, photon sans masse
-> relation a `G_F`
-> Yukawa et matrices de masse
-> architecture Saveur-Higgs.
```

Cette trajectoire reste conceptuelle.

Elle indique l'ordre de constitution du regime dans le corpus.
...
| `G_F` | route d'acces | reconstruit `v` par phenomenologie basse energie |
| `M_W`, `M_Z` | temoins phenomenologiques | controlent la distribution de `v` par les couplages de jauge |
| photon | residu electromagnetique | fixe le regime `U(1)_em` |
...
```text
structure theorique + acces phenomenologique + effet architectural.
```
...
```text
Une constance de seuil est un mode de stabilisation dans lequel une
grandeur d'echelle rend un regime physiquement lisible, coordonne
...
`M_W` et `M_Z` portent le controle faible ;
`G_F` porte une route d'acces basse energie ;
les Yukawa portent le relais vers la differenciation fermionique.
```

La qualification obtenue est donc :

```text
`v` est candidat fort a la constance de seuil du regime
electrofaible.
```

Le cycle donne au seuil le rang central de stabilisation et situe
chaque grandeur dans ce seuil.

### 8. Articulation avec Saveur-Higgs

Saveur-Higgs stabilise la constitution interne du regime deja brise :
...
Les deux cycles se raccordent par `v`.
...
> Le seuil BEH donne au regime electrofaible une constance de seuil :
> `v` y fonctionne comme noyau d'echelle, les masses faibles comme
> temoins phenomenologiques, `G_F` comme route d'acces et les
> Yukawa comme relais vers l'architecture Saveur-Higgs.
