# Note de cadrage BEH - zone de jonction v0.1

## Brout-Englert-Higgs, Saveur-Higgs, seuil electrofaible et historicite cosmique

### 0. Statut documentaire

```text
statut : note de cadrage ;
objet : mecanisme Brout-Englert-Higgs comme zone de jonction ;
fonction : poser le sas d'ouverture prudente du cycle du seuil
electrofaible ;
registre : positif, conditionnel, preparatoire.
```

Cette note remplit une fonction de sas documentaire.

Elle prepare un cycle BEH ulterieur en fixant les objets candidats,
les niveaux de lecture et les conditions d'entree.

Elle situe les operations plus fortes - cycle complet, these physique
et extraction de resultat - comme etapes posterieures du travail.

Elle fixe le seuil de travail a partir duquel le corpus pourra instruire
la brisure electrofaible en distinguant :

```text
1. le mecanisme physique a documenter ;
2. l'architecture Saveur-Higgs deja active ;
3. le debat philosophique sur la brisure de jauge ;
4. la question Q2 sur la formation des constances.
```

Sources internes immediates :

```text
01_CADRE_METHODOLOGIQUE/Protocole_travail_redaction_post_philosophie_v0_1.md
02_CYCLES_PHYSIQUES/05_Cycle_saveur_Higgs/Synthese_cycle_saveur_Higgs_v0_1.md
04_ARCHITECTURES_INTER_FAMILLES/Cercle2_lot2A_Saveur_Higgs_v0_1.md
04_ARCHITECTURES_INTER_FAMILLES/Architecture_saveur_Higgs_notes.md
05_CARTES_ET_SYNTHESES/Mise_a_jour_intellectuelle_v0_1.md
05_CARTES_ET_SYNTHESES/Audit_integration_branche_philosophique_v0_1.md
06_PHILOSOPHIE/Releve_paysage_brisure_electrofaible_v0_1.md
```

### 1. Diagnostic de depart

Le cycle Saveur-Higgs dispose maintenant d'une architecture constitutive
robuste.

Sa chaine positive est :

```text
regime electrofaible brise
+ echelle constitutive
+ textures de couplage
+ diagonalisation
+ spectres de masse
+ orientations observables.
```

Son point de depart est le regime deja brise.

Elle utilise `v` comme echelle constitutive et lit les masses, les
textures et les orientations a partir de cette echelle.

Le point BEH se situe juste avant ce regime de lecture :

```text
BEH = seuil ou le regime devient lisible comme architecture
Saveur-Higgs.
```

La dette porte donc sur le point d'entree du cycle :

```text
qu'est-ce qui devient assez stabilise pour que le regime
Saveur-Higgs puisse commencer ?
```

### 2. Question positive de cadrage

Le debat sur la realite de la brisure de jauge fournit un contexte
de prudence.

Le cadrage retient une question de qualification :

```text
dans le passage BEH, quel objet acquiert un statut de constance
physique : l'echelle `v`, l'etat fondamental, les masses faibles,
les masses fermioniques, les couplages, le potentiel, ou la
description de brisure ?
```

Cette formulation maintient le projet dans son axe propre.

Elle transforme le debat sur la symetrie en probleme de
stabilisation disponible pour le corpus des constantes effectives
stabilisees.

### 3. Quatre plans a tenir separes

| Plan | Objet | Fonction dans le corpus | Prudence |
|---|---|---|---|
| Mecanisme physique | champ de Higgs, potentiel, etat fondamental, `v`, masses faibles | instruire le seuil electrofaible | a documenter avant conclusion |
| Architecture Saveur-Higgs | `v`, Yukawa, matrices de masse, diagonalisation, CKM, PMNS | regime deja brise | maintenir le seuil distinct des effets lisibles |
| Debat jauge / invariant | brisure, redondance de jauge, formulation gauge-invariant, FMS | clarifier le statut de la description | convertir le debat en controle de formulation |
| Q2 formation | apparition d'une constance de regime | preparer la question genetique | reste secondaire par rapport a Q1 |

Le cadrage BEH existe parce que ces quatre plans se recoupent.

Sa robustesse vient de l'equilibre entre leurs fonctions respectives.

### 4. Objets candidats a stabilisation

Le prochain travail physique devra distinguer plusieurs candidats.

| Candidat | Lecture positive | Dette d'instruction |
|---|---|---|
| `v` | echelle constitutive du regime electrofaible brise | statut exact, acces via `G_F`, dependance au cadre |
| Etat fondamental | configuration a partir de laquelle les masses deviennent lisibles | formulation gauge-invariant a controler |
| Masses faibles | masses de `W` et `Z` associees au regime brise | articulation avec `v` et couplages de jauge |
| Masses fermioniques | spectres issus de `v`, Yukawa et diagonalisation | deja traitees dans Saveur-Higgs, mais apres le seuil |
| Potentiel de Higgs | structure parametrique ouvrant le chantier scalaire | masse du Higgs, auto-couplage, stabilite du vide |
| Brisure | description de transition ou de regime | tenir comme nom de seuil avant qualification d'objet |

Decision provisoire :

```text
la brisure sert ici de nom de seuil ;
son eventuel statut d'objet stabilise reste a instruire.
```

Le candidat principal pour le corpus reste a instruire.

La piste la plus sobre est :

```text
BEH stabilise un regime d'echelle et de masses,
dont `v` fournit le noeud d'entree pour l'architecture Saveur-Higgs.
```

Cette phrase reste une hypothese de travail.

Elle attend la note physique.

### 5. Regime physique et regime d'acces

Le protocole post-philosophie impose de separer :

```text
regime physique ;
regime d'acces.
```

Pour BEH, la separation est decisive.

| Niveau | Lecture de travail |
|---|---|
| Regime physique | structure electroweak et etat a partir duquel l'echelle `v` et les masses deviennent pertinentes |
| Regime d'acces | ajustements electroweak, `G_F`, masses faibles, phenomenologie Higgs, mesures de collision |
| Regime de description | jauge fixee, formulation perturbative, formulations gauge-invariant |

Le releve philosophique signale que la brisure porte une dette
descriptionnelle.

La note physique devra donc attribuer a chaque formulation sa portee
propre : jauge fixee comme regime de description, formulation
gauge-invariant comme controle de statut.

### 6. Effet sur Saveur-Higgs

La note Saveur-Higgs reste valide.

Elle lit le regime brise :

```text
v -> Yukawa -> matrices de masse -> diagonalisation -> spectres
et orientations.
```

Le cadrage BEH ajoute seulement le seuil amont :

```text
seuil BEH -> disponibilite de `v` comme echelle de regime
-> architecture Saveur-Higgs.
```

Le controle porte sur le statut du premier terme :

```text
que signifie exactement "disponibilite de `v`" ?
```

### 7. Effet sur le seuil electrofaible et l'historicite cosmique

Dans `Mise_a_jour_intellectuelle_v0_1.md`, le cycle du seuil
electrofaible est identifie comme le point aveugle de la brisure
electrofaible.

Dans la branche philosophique, la jonction d'historicite cosmique
designe le raccord avec l'histoire thermique des regimes.

Le cadrage present les articule ainsi :

```text
cycle du seuil electrofaible = instruction physique du seuil BEH ;
jonction d'historicite cosmique = horizon philosophique et temporel
du meme seuil ;
la note presente = sas de jonction avant ouverture.
```

Le vocabulaire de jonction joue ici un role de sas.

Il donne au cycle du seuil son objet d'ouverture : le type d'objet
que le corpus cherche a qualifier.

### 8. Conditions d'entree avant ouverture du cycle du seuil

Avant d'ouvrir le cycle du seuil electrofaible, le corpus doit
produire ou reunir :

```text
1. une note physique froide sur le mecanisme BEH ;
2. une carte minimale des grandeurs : `v`, couplages de jauge,
   masse du Higgs, parametres du potentiel, masses `W` et `Z` ;
3. une clarification du statut de la brisure dans les formulations
   gauge-invariant et gauge-fixees ;
4. une articulation avec l'architecture Saveur-Higgs deja active ;
5. une decision sur l'objet principal de qualification :
   `v`, regime d'echelle, masses, potentiel, ou seuil descriptif.
```

La condition 3 a une fonction circonscrite.

Elle donne au debat philosophique une fonction de controle de
formulation et reserve aux sources physiques la fonction d'arbitrage
du mecanisme.

### 9. Formules de travail autorisees

Formules prudentes :

```text
BEH ouvre le regime dans lequel l'echelle `v` devient operative
pour les masses faibles et pour l'architecture Saveur-Higgs.

Le corpus doit instruire si la constance pertinente porte sur `v`,
sur le regime d'echelle, sur les masses produites, ou sur la
stabilisation de l'etat a partir duquel ces grandeurs deviennent
lisibles.

La brisure est traitee comme nom de seuil jusqu'a l'instruction de
son statut physique exact.
```

Formules reservees a une instruction ulterieure :

| Formulation forte | Condition d'assertion |
|---|---|
| realite physique de la brisure de jauge | controle du debat jauge / invariant et formulation physique precise |
| statut constantiel de la brisure | distinction etablie entre brisure, `v`, etat fondamental et masses |
| `v` comme reponse au cycle du seuil | comparaison accomplie entre `v`, regime d'echelle, masses et potentiel |
| arbitrage philosophique du mecanisme | transformation en controle de formulation, puis retour aux sources physiques |
| origine du regime brise par Saveur-Higgs | cycle physique BEH / Higgs instruit en amont de l'architecture |

### 10. Raccord produit

La piece physique ouverte depuis ce cadrage est :

```text
02_CYCLES_PHYSIQUES/05_Cycle_saveur_Higgs/Note_physique_BEH_mecanisme_v0_1.md
```

Fonction tenue :

```text
decrire le mecanisme BEH, ses grandeurs pertinentes, ses acces
experimentaux et ses formulations prudentes, avant toute extraction
de these.
```

Elle a ensuite fourni les criteres permettant d'ouvrir le cycle du seuil
electrofaible comme :

```text
cycle du seuil electrofaible ;
qualification de `v` comme noyau d'echelle ;
controle par les masses faibles ;
raccord vers l'architecture Saveur-Higgs ;
reserve explicite pour le chantier du secteur scalaire de Higgs.
```

### 11. Formule de cloture

```text
BEH prend ici le rang de seuil.
Il nomme le point ou l'architecture Saveur-Higgs recoit son echelle
d'entree. La note physique, le cycle du seuil electrofaible et sa
synthese qualifient cette entree sans transformer la brisure en these
autonome.
```
