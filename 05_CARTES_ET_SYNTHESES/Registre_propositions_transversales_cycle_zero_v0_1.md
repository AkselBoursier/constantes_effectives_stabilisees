# Registre des propositions transversales — cycle zéro v0.1
...
Chaque proposition reçoit cinq champs :
...
| P6 | Le statut de constance peut avoir une trajectoire | Matrice temporelle, situations historiques | Q2, formation, fixité dynamique | Robuste comme axe d'enquête | Raconter une histoire sans mécanisme ou sans preuve |
| P7 | L'accès participe à la qualification sans produire l'objet | Cosmologie, SI, critiques réalistes | T3, Massimi, coordination, décision objet/accès/constitution | Garde-fou robuste | En faire le centre philosophique général |
| P8 | Une borne ou tension doit garder son rang | Neutrinos, cosmologie, fiche des limites | Refondation et audit borné | Robuste comme discipline | Promouvoir un domaine admissible en constance de l'objet |
| P9 | Les limites explicites renforcent la solidité d'un énoncé | Fiche des limites | Non-thèses, cycles physiques, audits | Discipline méthodologique forte | Appeler toute ignorance « composante de robustesse » |
| P10 | Le SI est un cas de définition et de réalisation, non un paradigme universel | Cycle SI puis rééquilibrage | Critère de latitude, conventions | Acquis local fort | Laisser le SI guider l'ensemble du corpus |
| P11 | La question de formation est antérieure à la question de qualification | Source fondatrice, note génétique | Q2, BEH, fine-tuning, fixité dynamique | Résultat généalogique établi | Transformer l'antériorité historique en priorité ontologique universelle |
| P12 | Formation physique, épistémique et conceptuelle sont distinctes | Note génétique | H1, programme philosophique C2 | Distinction utile, symétrie à reprendre | Supposer que toute constante « se forme deux fois » au même degré |
| P13 | Une partie du fine-tuning apparent peut venir de cadrans faussement indépendants | Cycle 9 | Carte des dépendances, contingence résiduelle | Hypothèse testable forte | Dissoudre tous les réglages par reconstruction |
| P14 | Un mécanisme peut canaliser une contingence sans expliquer sa valeur | Fine-tuning, cycle 10 | Migration du réglage, valeur tardive | Acquis conceptuel robuste | Confondre stabilité de trajectoire et sélection de valeur |
...
### 3.1 Pouvoir de refus

Le pouvoir de refuser le statut de constante est un contrôle discriminant. Il
n'est pas le but philosophique principal du projet.

```text
statut correct : le cadre ne doit pas tout absorber ;
statut incorrect : le projet viserait principalement à réduire la liste des
constantes légitimes.
```

### 3.2 Hiérarchie objet > constitution > accès

Cette formule a utilement séparé trois niveaux, mais elle a confondu :

```text
une distinction analytique ;
une hiérarchie d'intérêt ;
un ordre de recherche ;
...
`H_0` est utile pour montrer la différence entre appellation historique,
paramètre d'état, reconstruction et tension. Il n'a pas à devenir le contre-exemple
central d'un projet qui ne cherche pas principalement à distribuer des permis de
constance.

## 4. Propositions à rouvrir

| Proposition | Pourquoi la rouvrir | Type de travail requis |
|---|---|---|
| H1 : couplage entre formation physique et épistémique | La symétrie initiale ne correspond plus à l'intérêt actuel ; des cas peuvent être fortement asymétriques | Test cas par cas, recherche de contre-exemples |
| Portée de rétroaction | Relie constitution, dépendance au chemin et fine-tuning | Carte du conditionné puis test diachronique |
...
-> différences physiques et fonctionnelles
-> régimes et dépendances
-> cas contrastés
...
-> trajectoires et historicité
-> contingence canalisée ou résiduelle
-> fixité dynamique
-> conséquences philosophiques contrôlées.
```

## 6. Règle d'usage

Aucune proposition du registre ne commande seule un document futur. Tout nouveau
texte doit déclarer :

```text
quelle proposition il mobilise ;
à quelle question elle répond ;
quel type d'ordre il adopte ;
quels contre-exemples ou limites restent actifs.
```

## 7. Formule de clôture

> Le registre ne choisit pas une phrase maîtresse. Il empêche qu'une phrase,
> devenue utile dans une phase, soit prise rétrospectivement pour l'intention
> entière du projet.