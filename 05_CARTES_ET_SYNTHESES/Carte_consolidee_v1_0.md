# Carte consolidee v1.0

## Carte de reference apres consolidation methodologique, architecture effective basse energie et architecture cosmologique

### 1. Fonction de la v1.0

La carte v1.0 consolide l'etat actuel du projet apres trois etapes :

```text
1. la carte consolidee v0.9 ;
2. la note de synthese methodologique v1.1 ;
3. les notes d'integration des architectures effective basse energie et cosmologique.
```

Elle remplace la v0.9 comme carte de reference.

La modification principale est :

```text
Architecture cosmologique : confirmee comme architecture inter-familles.
```

La seconde modification est :

```text
Observable reconstruite : fonction architecturale stabilisee.
```

La v1.0 ne modifie pas le nombre de familles superieures. Elle renforce la distinction entre :
...
La v1.0 ajoute une consequence pratique :

```text
une grandeur ne doit pas etre classee par sa seule forme mathematique.
```

Elle doit etre classee selon :

```text
1. ce qu'elle fait ;
2. le regime ou elle joue ce role ;
3. la maniere dont elle est connue ;
4. le mode par lequel elle est stabilisee ;
5. l'architecture eventuelle dans laquelle elle devient solidaire d'autres grandeurs.
```
...
### 3. Table de rang v1.0

La carte v1.0 retient quatre niveaux principaux.
...
| Echelle | Fixer ou reveler un ordre de grandeur structurant | v, M_W, Lambda_QCD, M_Pl |
| Relation | Stabiliser une liaison entre grandeurs, regimes ou composantes | m_p / m_e, Delta m^2, Omega_i, w |
...
| Convention | Recevoir une valeur exacte par definition ou composition definissante | k_B, N_A, R |

Ces familles restent le noyau taxonomique.

Elles ne suffisent pas toujours a decrire le role complet d'une grandeur. Les fonctions transversales et architecturales sont alors ajoutees sans creer de nouvelle famille.
...
La nouveaute stabilisee par rapport a v0.9 est :
...
Architectures reconnues en v1.0 :

| Architecture | Statut v1.0 | Corpus de reference |
|---|---|---|
...
K_J : relation exacte par composition
R_K : relation exacte par composition
```
...
H_0        : Parametre d'etat actuel, non constante fondamentale
...
### 25. Decisions stabilisees par la v1.0

La carte v1.0 stabilise dix decisions.
...
9. Observable reconstruite devient une fonction architecturale stabilisee.
10. Les tensions de donnees appartiennent au regime d'acces.
```
...
La carte v1.0 est une carte de reference, mais elle n'est pas une cloture du projet.

Trois chantiers restent ouverts.

#### 26.1 Architecture metrologique SI etendue

L'architecture SI est maintenant etendue avec :
...
```text
comment classer des constantes definissantes qui ont aussi un role physique structurant et relationnel ?
```

Premier document produit :
...
L'architecture cosmologique est confirmee, mais certains sous-cycles pourraient etre ouverts plus tard.

Ils devront rester prudents et ne pas transformer :
...
en familles taxonomiques.

#### 26.3 Mise en ordre documentaire
...
### 27. Prochaine etape recommandee

La prochaine etape logique apres l'integration architecturale SI est :

```text
produire une carte consolidee v1.1
```
...
> Comment mettre a jour la carte de reference avec l'architecture metrologique SI etendue ?

Cette etape devient pertinente parce que la fiche c / h / e a maintenant ete integree au niveau architectural.

### 28. Documents internes integres

Documents de reference integres dans la v1.0 :
...
### 29. Formule de cloture

La v1.0 peut etre resumee par cinq formules.
...
Formule specifique v1.0 :

> La robustesse d'une constante effective est toujours une robustesse situee : elle depend d'une fonction, d'un regime, d'un acces, d'un mode de stabilisation et parfois d'une architecture.
